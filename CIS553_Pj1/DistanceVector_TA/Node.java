...
public class Node {
	...
	Packet packet = new Packet(Packet.BROADCAST_ADDRESS, this.mAddr, 1,
		Protocol.PING_PKT, mSequenceNumber++, Utility.stringToByteArray("NEIGHBOR_DISCOVERY"));
	...
	Packet packet = new Packet(Packet.BROADCAST_ADDRESS, this.mAddr,
		Packet.MAX_TTL, Protocol.LINK_INFO_PKT, mSequenceNumber++,
		linkState.pack());
	...
	try {
		...
		Packet packet = new Packet(neighborAddr, this.mAddr, 1,
			Protocol.DV_INFO_PKT, mSequenceNumber++, mDistanceVector.pack());
	...
	try {
	    ...
	    Packet packet = new Packet(destAddr, this.mAddr, Packet.MAX_TTL,
		    Protocol.PING_PKT, mSequenceNumber++, Utility.stringToByteArray(message));
	...
	try {
		...
		Packet reply = new Packet(packet.getSrc(), this.mAddr,
			...
			mSequenceNumber++, packet.getPayload());
		if (PING_BROADCAST) {
		...
		Packet reply = new Packet(Packet.BROADCAST_ADDRESS, this.mAddr,
			1, Protocol.PING_REPLY_PKT, mSequenceNumber++, packet.getPayload());
