...
public class Crypto {
    ...
    public synchronized Key makeRijndaelKey() {
	try {
	    KeyGenerator rkg = KeyGenerator.getInstance("Rijndael","SunJCE");
	    rkg.init(128,rand);
    ...
    public synchronized  byte[] encryptRijndael(Serializable obj, Key k) 
	throws KeyException, IOException
    {
	try {
	    Cipher c = Cipher.getInstance("Rijndael/ECB/PKCS5Padding", "SunJCE");
    ...
    public synchronized  Object decryptRijndael(byte[] msgE, Key k) 
	throws KeyException, IOException
    {
	try {
	    Cipher c = Cipher.getInstance("Rijndael/ECB/PKCS5Padding","SunJCE");
