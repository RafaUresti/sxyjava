...
public class LinkState {
  ...
  public boolean equals(LinkState ls){
    
    HashSet<Integer> neighborSet1 = new HashSet<Integer>();
    HashSet<Integer> neighborSet2 = new HashSet<Integer>();
    if (this.getNeighbors() == null || ls.getNeighbors() == null || 
        this.getNeighbors().length != ls.getNeighbors().length)
      return false;
    for (int i = 0; i < this.getNeighbors().length; i++){
      neighborSet1.add(this.getNeighbors()[i]);
      neighborSet2.add(ls.getNeighbors()[i]);
    }
    return neighborSet1.equals(neighborSet2);
  }
  
  public boolean equal(LinkState ls){
    int[] currentNeighbors = this.getNeighbors();
    int[] newNeighbors = ls.getNeighbors();
    if(currentNeighbors.length == newNeighbors.length)
    {
      sortIntArr(currentNeighbors);
      sortIntArr(newNeighbors);
      for(int i=0; i<currentNeighbors.length; i++)
      {
        if(currentNeighbors[i] != newNeighbors[i])
        {
          return false;
        }
      }
  ...
  private void sortIntArr(int[] arr){
    int nvTmp;
