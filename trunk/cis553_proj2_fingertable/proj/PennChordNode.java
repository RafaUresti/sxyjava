...
public class PennChordNode
{
	...
	private final int FIX_FINGER_TIME = 400000;
	private final int FIX_FINGER_START_TIME = 400000; 
	...
	private final int mBit = 32; //bit length of key
	private long pow2ToMBit;
	private long[] ftIndexValue;
	private int[] ftAddr;
	private long[] ftAddrKey;
	private int[] ftAddrComp;
	private long[] ftAddrKeyComp;
	private long[] ftStandValue; //store 2 to i
	public PennChordNode(Node baseNode,int addr)
	{
		this.baseNode = baseNode;
		this.myAddr = addr;
		this.myAddrKey = genKeyForNode();
		this.ftStandValue = new long[mBit];
		this.ftIndexValue = new long[mBit];
		this.ftAddr = new int[mBit];
		this.ftAddrKey = new long[mBit];
	...
	private void initializePennChordNode() {
		...
		this.inRing = false;
		ftAddrComp = new int[mBit];
		ftAddrKeyComp = new long[mBit];
		...
		for(int i=0; i<mBit; i++)
		{
			...
			ftAddrComp[i] = -1;
			ftAddrKeyComp[i] = -1;
			ftStandValue[i] = pow2ToMBit;
			pow2ToMBit = pow2ToMBit * 2;
		}

		long pow = 1;
		for(int i=0; i<mBit; i++)
		{
			ftIndexValue[i] = (this.myAddrKey + pow) % pow2ToMBit;
			pow = pow * 2;
		}
	}
	/**
	...
	public void processPacket(int from, Packet packet)
	{
		...
		else if (PennChordMessage.MessageType.FIX_FINGERS == (pennChordMsg.getMessageType())){
			receiveFixFingers(pennChordMsg);
		}
		else if (PennChordMessage.MessageType.FIX_FINGERS_REPLY == (pennChordMsg.getMessageType())){
			receiveFixFingersReply(pennChordMsg);
		}
		else {
	...
	private void receiveChangeSuccessor(PennChordMessage pennChordMsg) {
		successor = (int) pennChordMsg.getArgument1();
	...
	private void receiveChangePredecessor(PennChordMessage pennChordMsg) {
		predecessor = (int) pennChordMsg.getArgument1();
	...
	{ 
		...
		if(entryNodeAddr == this.myAddr)
		{
			...
			this.inRing = true;
			baseNode.addTimer(FIX_FINGER_START_TIME, "fixFingers");
		}else{
	...
	private void receiveJoin(Packet packet){
		...
		if (srcKey == myAddrKey || srcKey == successorKey){//if there is addrKey duplication, send a join_reply with -1 as auguments
			...
			PennChordMessage chordMessage = new PennChordMessage(myAddr, myAddrKey,
					PennChordMessage.MessageType.JOIN_REPLY, -1, -1, null, null, null, null);
			byte[] packedChordMessage = pack(chordMessage);
	...
	private void receiveJoinReply(PennChordMessage pcm){
		...
		baseNode.addTimer(FIX_FINGER_START_TIME, "fixFingers");//TODO check if right
	...
	public void store(String key,String value)
	{
		...
		if (isKeyMine(lookupKey) ){
			...
			PennChordMessage chordMessage = new PennChordMessage(myAddr, myAddrKey,
					...
					key, valueList, null, null);   // change the packet structure... added myAddrkey and for srcAddrKey and for argAddrkey

			//query finger table
			//      int ftSuccessor = getSuccessorFromFingerTable(lookupKey);
			//      int targetNodeAddr = this.successor;
			//      if(ftSuccessor > -1)
			//      {
			//        targetNodeAddr = ftSuccessor;
			//      }

			byte[] packedChordMessage = pack(chordMessage);
			Packet storePacket = new Packet(this.successor, myAddr, Packet.MAX_TTL, Protocol.CHORD_PKT, 
	...
	public void lookup(String key)
	{
		...
		if (isKeyMine(lookupKey)){
			...
			PennChordMessage chordMessage = new PennChordMessage(myAddr, myAddrKey,
					...
					key, null, null, null);        // change the packet structure... added myAddrkey and for srcAddrKey and for argAddrkey
			if (lookupDebugOn){
			...
			byte[] packedChordMessage = pack(chordMessage);
			Packet lookupPacket = new Packet(targetNodeAddr, myAddr, Packet.MAX_TTL, Protocol.CHORD_PKT, 
					baseNode.assignSequenceNumber(), packedChordMessage);
		...
		while(keyData.hasNext()){
			...
			if ((keyHash <= pcm.getSrcAddrKey() && keyHash > pcm.getArgument1())/*normal case*/
				...
				pcm = new PennChordMessage(myAddr, myAddrKey, 
						...
						0, 0, keyToSend, dataToSend, null, null);
	...
	private void notifyPredecessor() {
		PennChordMessage pcm = new PennChordMessage(myAddr, myAddrKey,
				...
				successor, successorKey, null, null, null, null);
		byte[] packedPcm = pack(pcm);
	...
	private void notifySuccessor() {
		PennChordMessage pcm = new PennChordMessage(myAddr, myAddrKey, 
				...
				predecessor, predecessorKey, null, null, null, null);
		byte[] packedPcm = pack(pcm);
	...
	private void transferData() {
		...
		while(data.hasNext()){
			...
			PennChordMessage pcm = new PennChordMessage(myAddr, myAddrKey, 
					PennChordMessage.MessageType.TRANSFER_DATA, 
					0, 0, transferKey, transferValues, null, null);
			byte[] packedPcm = pack(pcm);
			Packet transferPacket = new Packet(successor, myAddr, Packet.MAX_TTL, Protocol.CHORD_PKT, 
					baseNode.assignSequenceNumber(), packedPcm);
			sendPacket(successor, transferPacket);
	...
	private void requestLookupHops(int lookupCount, int hopCount, int src, long srcKey) {
		...
		pcm = new PennChordMessage(src, srcKey, 
				...
				lookupCount, hopCount, null, null, null, null);
		byte[] packedPcm = pack(pcm);
		Packet debugRequestPacket = new Packet(successor, myAddr, Packet.MAX_TTL, Protocol.CHORD_PKT, 
				baseNode.assignSequenceNumber(), packedPcm);
		sendPacket(successor, debugRequestPacket);
	...
	public boolean onCommand(String command)
	{
		...
		else if (command.startsWith("fingerstate"))
		{
			if (isRoutingTableComplete()){
				printFingerTable();
			}
	...
	private void printFingerTable() {
		if(ftAddrComp == null)
		{
			baseNode.logOutput("Finger table of node " + this.myAddr + " (Key=" + this.myAddrKey + ") is incomplete.");
		}else
		{
			baseNode.logOutput("Finger table of node " + this.myAddr + " (Key=" + this.myAddrKey + "):");
			baseNode.logOutput("<i, finger ID, finger address, finger address key>");

			int tmpI;
			for(int i=0; i<mBit; i++)
			{
				tmpI = i+1;
				baseNode.logOutput("<" + tmpI + ", " + ftIndexValue[i] + ", " 
						+ ftAddrComp[i] + ", " + ftAddrKeyComp[i] + ">" );
			}
		}   
	}

	private void printFingerTable1() {
		...
		for(int i=0; i<mBit; i++)
		{
			tmpI = i+1;
			baseNode.logOutput("<" + tmpI + ", " + ftIndexValue[i] + ", " 
					+ ftAddr[i] + ", " + ftAddrKey[i] + ">" );
		}

	}

	private void receiveFixFingers(PennChordMessage pennChordMsg) {
		//baseNode.logOutput("Receive fix_fingers packet");
		long srcAddrKey = pennChordMsg.getSrcAddrKey();
		int idx = (int) pennChordMsg.getArgument1();
		long ftEntryValue = (srcAddrKey + ftStandValue[idx]) % pow2ToMBit;

		while(idx<mBit && isKeyMine(ftEntryValue))
		{
			ftEntryValue = (srcAddrKey + ftStandValue[idx]) % pow2ToMBit;
			//update the entry
			pennChordMsg.ftAddr[idx] = this.myAddr;
			pennChordMsg.ftAddrKey[idx] = this.myAddrKey;
			idx++;
		}

		if(idx == mBit)
		{
			//      baseNode.logOutput("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$send finger reply");
			//Complete. Send fix_finger_reply to the src
			PennChordMessage chordMessage = new PennChordMessage(pennChordMsg.getSrc(), 
					pennChordMsg.getSrcAddrKey(),
					PennChordMessage.MessageType.FIX_FINGERS_REPLY, -1, -1, null, null, 
					pennChordMsg.ftAddr, pennChordMsg.ftAddrKey);
			//baseNode.logOutput("sending a fix_fingers_reply packet to: " + chordMessage.getSrc());     
			if (successor >= 0 && baseNode.isLinkStateReady()){
				byte[] packedChordMessage = pack(chordMessage);
				Packet fixPacket = new Packet(pennChordMsg.getSrc(), this.myAddr, Packet.MAX_TTL,
						Protocol.CHORD_PKT, baseNode.assignSequenceNumber(), packedChordMessage);
				sendPacket(pennChordMsg.getSrc(), fixPacket);
			}
		}else
		{
			//Incomplete. Pass the packet to the successor
			//      baseNode.logOutput("$$$$$$$$$$$$$$$$$$$$$$$ sending a fix_fingers packet to: ");
			PennChordMessage chordMessage = new PennChordMessage(pennChordMsg.getSrc(), 
					pennChordMsg.getSrcAddrKey(),
					PennChordMessage.MessageType.FIX_FINGERS, idx, -1, null, null, 
					pennChordMsg.ftAddr, pennChordMsg.ftAddrKey);
			if (successor >= 0 && baseNode.isLinkStateReady()){
				byte[] packedChordMessage = pack(chordMessage);
				Packet fixPacket = new Packet(this.successor, this.myAddr, Packet.MAX_TTL,
						Protocol.CHORD_PKT, baseNode.assignSequenceNumber(), packedChordMessage);
				sendPacket(this.successor, fixPacket);
			}
		}
	}
    ...
    for(int i=0; i<mBit; i++)
    {
      ftEntryValue = (srcAddrKey + ftStandValue[i]) % pow2ToMBit;
      if(ftEntryValue > this.myAddrKey 
          && (ftEntryValue <= this.successorKey || isLargestNode()))
      {
        //update the entry
        pennChordMsg.ftAddr[i] = this.successor;
        pennChordMsg.ftAddrKey[i] = this.successorKey;
      }
      if(pennChordMsg.ftAddr[i] > -1)
      {
        completeCnt++;
      }
    }
    ...
    {
      //Incomplete. Pass the packet to the successor
      //baseNode.logOutput("sending a fix_fingers packet to: " + chordMessage.getSrc());
      PennChordMessage chordMessage = new PennChordMessage(pennChordMsg.getSrc(), 
          pennChordMsg.getSrcAddrKey(),
          PennChordMessage.MessageType.FIX_FINGERS, -1, -1, null, null, 
          pennChordMsg.ftAddr, pennChordMsg.ftAddrKey);
      if (successor >= 0 && baseNode.isLinkStateReady()){
        byte[] packedChordMessage = pack(pennChordMsg);
        Packet fixPacket = new Packet(this.successor, pennChordMsg.getSrc(), Packet.MAX_TTL,
            Protocol.CHORD_PKT, baseNode.assignSequenceNumber(), packedChordMessage);
        sendPacket(this.successor, fixPacket);
      }
    }
	...
	private void receiveFixFingersReply(PennChordMessage pennChordMsg) {
		this.ftAddrComp = pennChordMsg.ftAddr;
		this.ftAddrKeyComp = pennChordMsg.ftAddrKey;    
	}



	private int getSuccessorFromFingerTable(long qryKey)
	{
//		if (true) return -1;
		for (int i = 0; i < mBit; i++){
			if (ftAddrKeyComp[i] < 0)
				return -1;
		}
		for(int i = mBit -1 ; i>0; i--)
		{
			
			if((qryKey >= ftAddrKeyComp[i-1] && qryKey < ftAddrKeyComp[i]) 
				    		  || qryKey >= ftAddrKeyComp[i-1] && ftAddrKeyComp[i] < ftAddrKeyComp[i-1])

			{
				return ftAddrComp[i-1];
			}
		}
		if (qryKey >= ftAddrKeyComp[31]){
			return ftAddrComp[31];
		}
	...
	public void fixFingers(){
		//baseNode.logOutput("fixFingers called");
		if (isOnlyNode()){//I am the only node in the ring
			for(int i=0; i<mBit; i++)
			{
				this.ftAddr[i] = this.myAddr;
				this.ftAddrKey[i] = this.myAddrKey;
			}
		...
		}else
		{
			...
			if(successor >=0)
			{
				//check entries < this.successorKey
				int idx=0;
				while(idx<mBit && ftIndexValue[idx] > this.myAddrKey 
						&&(ftIndexValue[idx] <= this.successorKey|| isLargestNode()))
				{
					ftAddr[idx] = this.successor;
					ftAddrKey[idx] = this.successorKey;
					idx++;
				}

				PennChordMessage chordMessage = new PennChordMessage(myAddr, myAddrKey,
						PennChordMessage.MessageType.FIX_FINGERS, idx, -1,
						null, null, this.ftAddr, this.ftAddrKey);
				...
				if (successor >= 0 && baseNode.isLinkStateReady()){
					Packet predecessorPacket = new Packet(successor, myAddr, Packet.MAX_TTL, Protocol.CHORD_PKT, 
							baseNode.assignSequenceNumber(), packedChordMessage);
					sendPacket(successor, predecessorPacket);
					//baseNode.logOutput(this.myAddr + ": send TEST packet " + i);
				}

			}
		}

		baseNode.addTimer(FIX_FINGER_TIME, "fixFingers");//TODO check if right
	}
}
