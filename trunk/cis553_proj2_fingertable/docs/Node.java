...
public class Node {
	...
	private void introduce() {
		Packet packet = new Packet(Packet.BROADCAST_ADDRESS, this.addr, Packet.MAX_TTL,
				Protocol.NAME_PKT, this.currentSequenceNumber, Utility.stringToByteArray(alias));
	...
	private boolean matchPingCommand(String command) {
		...
		try {
			...
			Packet packet = new Packet(destAddr, this.addr, Packet.MAX_TTL, Protocol.PING_PKT, this.currentSequenceNumber,
	...
	public void neighborDiscovery() {
		...
		Packet packet = new Packet(Packet.BROADCAST_ADDRESS, this.addr, 1, Protocol.PING_PKT, this.currentSequenceNumber, payload);
		this.currentSequenceNumber++;
		this.send(Packet.BROADCAST_ADDRESS, packet);
		this.addTimer(PingTimeout, "neighborDiscovery");
	}
	...
	private void receivePing(Packet packet) {
		...
		if (!this.isValidSequence(packet)) {
			return;
		}
		try {
			//if the packet is for us
			if (packet.getDest() == this.addr) {
				Packet reply = new Packet(packet.getSrc(), this.addr, Packet.MAX_TTL, Protocol.PING_REPLY_PKT, this.currentSequenceNumber, packet.getPayload());
