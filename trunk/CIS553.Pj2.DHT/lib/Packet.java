...
public class Packet
{
     ...
     * 
     * @param dest
     *            The destination fishnet address.
     * @param src
     *            The source fishnet address.
     * @param ttl
     ...
     */
    public Packet(int dest, int src, int ttl, int protocol, int seq, byte[] payload)
	throws IllegalArgumentException
    {
    ...
    {
	return new String("Packet: " + this.src + "->" + this.dest + " protocol: " + this.protocol
			  + " TTL: " + this.ttl + " seq: " + this.seq + " contents: "
			  + Utility.byteArrayToString(this.payload));
    }
     ...
     * Format: destination address: 1 byte source address: 1 byte ttl (time to
     * live): 1 byte protocol: 1 byte packet length: 1 byte packet sequence num:
     * 4 bytes payload: <= MAX_PAYLOAD_SIZE bytes
     * 
	...
	byteStream.write(intToByteArray(this.payload.length + Packet.HEADER_SIZE),0,4);
    ...
    /**
     * Unpacks a byte array to create a Packet object Assumes the array has been
     * formatted using pack method in Packet
     * 
     * @param packedPacket
	...
	byteStream.read(pLength, 0, 4);
	int packetLength = byteArrayToInt(pLength, 0);
	...
	if ((9 + payload.length) != packetLength)
	    {System.out.println("FOUND NULL 2 " + payload.length + " "+packetLength);
	    return null;
	    }
    ...
    {
	return this.isValid(this.dest, this.src, this.ttl, this.protocol, this.payload.length
			    + HEADER_SIZE);
    }

    private boolean isValid(int dest, int src, int ttl, int protocol, int size)
    {
	return (dest <= MAX_ADDRESS && dest >= 0 && Packet.validAddress(src)
		&& Protocol.isProtocolValid(protocol) && ttl <= MAX_TTL && ttl >= 0 && size <= MAX_PACKET_SIZE);

    }

    /**
     * Tests if this Packet is valid to send A valid packet may have TTL = 0,
     * but a "valid to send" packet cannot have TTL = 0
     * 
