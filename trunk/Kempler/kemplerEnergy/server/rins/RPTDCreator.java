...
//	String baseURL = "http://209.97.223.189:8080/RINs/DocRetrieve?filePath=";
	...
	private String fileDir = "c:\\KemplerFiles\\rptd\\";
	...
	public RPTDCreator( EntityManager em ){
		this.em = em;
	}
	...
	private void splitInvoice(Invoice iv){
		...
		Collections.sort(rinsList1, new Comparator<RIN>(){
		...
		Collections.sort(rinsList2, new Comparator<RIN>(){
		...
		Collections.sort(retiredRinsList1, new Comparator<RIN>(){
		...
		Collections.sort(replacedRinsList1, new Comparator<RIN>(){
		...
		Collections.sort(retiredRinsList2, new Comparator<RIN>(){
		...
		Collections.sort(replacedRinsList2, new Comparator<RIN>(){
	...
	public String[] makeRPTD(Invoice iv) throws Exception {
...
//			iv.setRptdK2(null);
	...
	private void generateRPTD(int type, Invoice iv, String filePath) throws Exception {//, List<RIN> arrayRIN ){
...
//		System.out.println("type is "+type);
		...
		if(iv.getInvoiceType().equalsIgnoreCase("PURCHASE")){
			...
			cszTransferee = "New York, NY, 10279";
			...
			companyTransferor = iv.getEpaPartner().getFullName();
			...
			if (RIV_r != null) {
				...
				if (type == 1)// k1
					for (int i = 0; i < retiredRinsList1.size(); i++) {
						System.out.println("retiredRinsList1: " + retiredRinsList1.get(i).toString());
					...
					for (int i = 0; i < retiredRinsList2.size(); i++) {
						System.out.println("retiredRinsList2: " + retiredRinsList2.get(i).toString());
			...
			if (riv != null) {
				...
				if (type == 1)
					for (int i = 0; i < rinsList1.size(); i++) {
						System.out.println("rinsList1: [" + ((RIN)rinsList1.get(i)).getSaleIndex()+ "] "+ rinsList1.get(i).toString());
					...
					for (int i = 0; i < rinsList2.size(); i++) {
						System.out.println("rinsList2: " + rinsList2.get(i).toString());
			...
			if (riv_replaced != null) {
				...
				if (type == 1)
					for (int i = 0; i < replacedRinsList1.size(); i++) {
						...
						System.out.println("replacedRinsList1: " + replacedRinsList1.get(i).toString());
					...
					for (int i = 0; i < replacedRinsList2.size(); i++) {
						...
						System.out.println("replacedRinsList2: " + replacedRinsList2.get(i).toString());
