...
public class SearchPanel extends Composite{
	...
	private final SuggestBox epaPartnerNameBox = epaPartnerSuggestionPanel.getEpaPartnerNameBox();
	...
	final int[] RESULT_WIDTHS = new int[]{30, 200, 300, 150, 80, 80};
...
//	final String TITLE_WIDTH = TOTAL_WIDTH +"";
	final static String URL = "http://209.97.223.189:8080/RINs/DocRetrieve?filePath=";
	...
	private void assembleInputPanel() {
		...
		/*
		 * Drop down box to choose invoice type: PURCHASE/SALE/BOTH
		 * and record into RINInvoiceSearch object
		 */
		 ...
		 * which disappears upon click and change. Change stored in RINInvoiceSearch object
		 */
		...
		invoiceNumberBox.addChangeListener(new ChangeListener(){
			public void onChange(Widget arg0) {
				...
				else invoiceNumberBox.setText(INVOICE_NUMBER);
			}
		});

		/*
		 * EPA partner box. Sets EPA partner to RINInvoiceSearch object at change
		 */
		
		epaPartnerSuggestionPanel.getEpaPartnerPanel().remove(epaPartnerSuggestionPanel.getEpaPartnerNameLabel());
		...
		/*
		 * From date box and calendar. Sets from date to RINInvoiceSearch object at change
		 */
		...
		fromDateCalendar.addChangeListener(new ChangeListener(){
			public void onChange(Widget arg0) {
				...
				String fromDate = fromDateCalendar.getYear()+"-"+month+"-"+fromDateCalendar.getDay();
		...
		/*
		 * To date box and calendar. Sets from date to RINInvoiceSearch object at change
		 */
		...
		toDateCalendar.addChangeListener(new ChangeListener(){
			public void onChange(Widget arg0) {
				...
				String toDate = toDateCalendar.getYear()+"-"+month+"-"+toDateCalendar.getDay();
	...
	private void assembleSearchClearPanel() {
		...
		searchClearPanel.setHorizontalAlignment(HasHorizontalAlignment.ALIGN_CENTER);
		...
		searchButton.addClickListener(new ClickListener(){
			public void onClick(Widget arg0) {
				if (!epaPartnerNameBox.getText().equalsIgnoreCase(EPA_PARTNER) 
					...
					search.setEpaPartner(epaPartnerSuggestionPanel.getEPAPartner());
				}
				searchButton.setEnabled(false);
				svc.getInvoiceList(search, new AsyncCallback<ArrayList<Invoice>>(){
					public void onFailure(Throwable arg0) {
						Window.alert("Search failure: "+ arg0.getMessage());
				...
				/*svc.getInvoice(search, new AsyncCallback<Invoice>(){
					public void onFailure(Throwable arg0) {
						Window.alert("failed to get invoice");
					}
				...
				i1.setDefault();
				i1.setInvoiceType("SALE");
				i1.setInvoiceNo("Fake invoice");
				i1.getEpaPartner().setFullName("Dammy counterParty how are tyou djdjahd jjdfa fdlfjal;k flasdja sdfjk");
				i1.setInvoiceDate("2000-1-1");
				invoices.add(i1);

				Invoice i2 = new Invoice();
				i2.setDefault();
				i2.setInvoiceType("SALE");
				i2.setInvoiceNo("Drake invoice");
				i2.getEpaPartner().setFullName("Tummy counterParty");
				i2.setInvoiceDate("2008-1-1");
				invoices.add(i2);
				...
				i3.setInvoiceDate("2008-2-1");
				invoices.add(i3);

				for (int j = 0; j < 100; j++){
					Invoice x = new Invoice();
					x.setDefault();
					x.setInvoiceNo("Lake invoice"+j);
					i2.getEpaPartner().setFullName("Cummy counterParty"+j);
					x.setInvoiceDate("2008-2-"+j);
					invoices.add(x);
				}
				resultsPanel.displayInvoices(invoices);*/
			}
					...
					if (sortOrders[0]){
						...
						Collections.sort(invoices, new Comparator<Invoice>(){
							public int compare(Invoice i1, Invoice i2) {
								return i2.getInvoiceType().compareTo(i1.getInvoiceType());
							}
						});
						resultsPanel.displayInvoices(invoices);
					}
						...
						Collections.sort(invoices, new Comparator<Invoice>(){
							public int compare(Invoice i1, Invoice i2) {
								return i1.getInvoiceType().compareTo(i2.getInvoiceType());
							}
						});
						resultsPanel.displayInvoices(invoices);
					}
			...
			public void onClick(final Widget arg0) {
				if (invoices != null && !invoices.isEmpty()){
					if (sortOrders[1]){
						...
						Collections.sort(invoices, new Comparator<Invoice>(){
							public int compare(Invoice i1, Invoice i2) {
								return i2.getInvoiceNo().compareTo(i1.getInvoiceNo());
							}
						});
						resultsPanel.displayInvoices(invoices);
					}
						...
						Collections.sort(invoices, new Comparator<Invoice>(){
							public int compare(Invoice i1, Invoice i2) {
								return i1.getInvoiceNo().compareTo(i2.getInvoiceNo());
							}
						});
						resultsPanel.displayInvoices(invoices);
					}
			...
			public void onClick(final Widget arg0) {
				if (invoices != null && !invoices.isEmpty()){
					if (sortOrders[2]){
						...
						Collections.sort(invoices, new Comparator<Invoice>(){
							public int compare(Invoice i1, Invoice i2) {
								return i2.getEpaPartner().getFullName().compareTo(i1.getEpaPartner().getFullName());
							}
						});
						resultsPanel.displayInvoices(invoices);
					}
						...
						Collections.sort(invoices, new Comparator<Invoice>(){
							public int compare(Invoice i1, Invoice i2) {
								return i1.getEpaPartner().getFullName().compareTo(i2.getEpaPartner().getFullName());
							}
						});
						resultsPanel.displayInvoices(invoices);
					}
...
//		resultsPanel.setSize(TITLE_WIDTH, RESULTS_HEIGHT);
	...
	private class SearchResult extends HorizontalPanel{
		...
		SearchResult(Invoice invoice){
			final Label psLabel = new Label(invoice.getInvoiceType().equalsIgnoreCase("PURCHASE")? "P" : "S");
			psLabel.setWidth(PS_WIDTH);
			final Label invoiceLabel = new Label(invoice.getInvoiceNo()+"_v"+invoice.getVersionNo());
			invoiceLabel.setWidth(INVOICE_WIDTH);
			final Label counterPartyLabel = new Label (invoice.getEpaPartner().getFullName());
			...
			final String rptdURL = "<a href=\"" + URL + invoice.getRptdPath() + "\" target=\"_blank\">RPTD</a>";
			...
			final String csvURL = "<a href=\"" + URL + invoice.getCsvPath() + "\" target=\"_blank\">CSV</a>";
