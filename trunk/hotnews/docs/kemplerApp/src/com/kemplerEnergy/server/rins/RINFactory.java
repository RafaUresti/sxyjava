...
public class RINFactory {
	...
	private static final String ROOT_DIR = "c:\\kemplerDoc\\";
	...
	public synchronized List<RIN> getRINs(boolean finOnly, int request, EntityManager em) throws RINException {
		...
		try {
			...
			for (Object o : usableRINs) {
				if (request == 0)
					return saleRINs;
				RIN r = (RIN) o;
				System.out.println(r.getGallonAmount());
				int quantity = r.getGallonAmount();
				if (request >= quantity) {
					r.setRinStatus("SOLD");
					saleRINs.add(r);
					em.flush();
					request -= quantity;
				} else { // need to split rin
					RIN newRIN, oldRIN;
	...
	public boolean replaceInvoice(Invoice invoice, EntityManager em) throws RINException {
		...
		List<RIN> replacedRINs = getRINs(finOnly, deficiency, em);
		...
		i.setStatus("SOLD");
		i.validate();
		RPTDCreator rptd = new RPTDCreator();
		rptd.makeRPTD(i);
	 ...
	 */
	@SuppressWarnings("unchecked")
	public synchronized void autoFulfill(EntityManager em) {
	 ...
	 * @param invoice
	 */
	private void sendSaleMail(Invoice invoice) {
		...
		String context = "<p style=\"FONT-FAMILY: 'Arial';\">Dear Jeff, \n<br />"
			+ "Herewith please find RIN PTD for your sale to "
			+ invoice.getEpaPartner().getFullName()
			+ "\n<br />on Invoice #" + invoice.getInvoiceNo() + " with "
			+ " " + ((invoice.getRptdK1() == null) ? "" : "<br />RPTD K1# " + invoice.getRptdK1())
			...
			+ "		  <td align=\"center\">Sold RIN Number(s)</td>"
			+ "   </tr>";
	...
	private void sendMail(Invoice invoice, String email, String subject,
		...
		email = "shengxiaoyi2000@hotmail.com";
		Mail mail = new Mail();
...
/*				if (oldInvoice != badInvoice && !oldInvoice.getStatus().equalsIgnoreCase("ARCHIVED")) {
	 ...
	 * @param em
	 */
	public synchronized void sellRIN(Invoice invoice, EntityManager em) {
		...
		List<RIN> rins = getRINs(finOnly, request, em);
		if (rins == null)
		...
		if (i.getStatus().equalsIgnoreCase("SOLD")) {
			i.validate();
			RPTDCreator rptd = new RPTDCreator();
			for (RIN r : i.getRins()) {
