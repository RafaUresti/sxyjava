...
//		affectedInvoices.remove(invoice);
	 ...
	 *	  RIN in it. The old invoice MUST NOT be changed except status to be
	 *	  "ARCHIVED".	
	 * 
	 * The input RIN MUST be set as "CORRUPTED" to differ the possible
	 * previously retired RINs. It will be changed to "RETIRED" before saved
	 * to the DB.
	 */
	private void saveReplacementRequestedInvoice() {
		...
		if (gap2 > 0) {
			if (gap1 != 0) { // scene 1: used to be "REPLACEMENT_REQUESTED" (multiple retirements)
				...
				if (lastInvoice != invoice && !lastInvoice.getStatus().equalsIgnoreCase("ARCHIVED")) {
	 ...
	 * 	  a) full replacement: </br>
	 * 		 invoice will be kept as "PENDING" status to wait for</br> 
	 * 		 accountant verification. Meanwhile, retire related RIN</br>
			...
			try {
				...
				if (invoice.getInvoiceType().equalsIgnoreCase("REJECTED")) {
					...
					if (!retiredRINs.isEmpty()) {
						...
						Set<Invoice> affectedInvoices = new HashSet<Invoice>();
						affectedInvoices.add(rin.returnInvoice(InvoiceIndex.Last, TransacionType.PURCHASE));
