 ...
 */
	...
	/**
	 * Prints the ocean and its status after each shot
	 */
	void print() {
		for (int j = 0; j<sea.length; j++)
		System.out.print("   "+j);//prints the column numbers
		System.out.println();
		printRowDivider();
   ...
   */
    ...
    /**
     * Prints each row of the ocean
     * @param row The row number to be printed
     */
	void printRow(int row) {
		char hiddenShips; //replaces ships and adjacent spaces w/ empty locations in printing
		for (int j=0; j<sea[0].length; j++) {
			hiddenShips=sea[row][j];
			if (sea[row][j]!='X'&& sea[row][j]!='o')
				hiddenShips=' ';
			System.out.print(" "+hiddenShips+" |");
		}
		System.out.println();
	}
