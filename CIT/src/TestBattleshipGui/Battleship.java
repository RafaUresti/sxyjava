...
public class Battleship {
	...
	private void run(Battleship game) {
		...
		do {
			...
			gui = new BattleshipGui(game);
									
			while (!ocean.isGameOver()) {
				playGame();
			}
			System.out.println("Congratulations! You have destroyed the entire enemy fleet!");
			System.out.println("Your score is: "+ocean.numberOfShotsFired()+" shots and "+ocean.numberOfHits()+ " hits.");
	...
	private void playGame() {
		...
		System.out.println();
		if (hitTarget) {
			gui.markHit(row, column);
			System.out.println("Good shot!\n");
		}			
		else {
			gui.markMiss(row, column);
