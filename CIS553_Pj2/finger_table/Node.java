...
public class Node {
        ...
        public final long PingTimeout = 30000;  // Timeout pings in 10 seconds
        ...
        public void start() {
                ...
                this.addTimer(PingTimeout*2,"linkStateTimeOut");
                this.addTimer(PingTimeout*3, "floodLinkState");
        }
                ...
                if (!(packet.getDest() == this.addr || packet.getDest() == Packet.BROADCAST_ADDRESS)) {
                        ...
                        if (packet.getTTL() > 0) {
                                try{
                                  ...
                                  if(confirmedList.get(packet.getDest()) == null)
                                  {
                                    /*this.logOutput("confirmedList.get(packet.getDest()) == null");
                                    this.logOutput("Packet Dest = " + packet.getDest()); 
                                    this.logOutput("Packet Protocol = " + packet.getProtocol());
                                    PennChordMessage objPCM = PennChordNode.unpack(packet.getPayload());
                                    this.logOutput("Msg type = " + objPCM.getMessageType());*/
                                    this.send(Packet.BROADCAST_ADDRESS, packet);
                                  }else
                                  {
                                    int nextHop = confirmedList.get(packet.getDest()).nextHop;
        ...
        private void receiveLinkState(Packet packet) {
                ...
                LinkState currentLinkState = linkStates.get(packet.getSrc());
                ...
                if (currentLinkState == null || !newLinkState.equals(currentLinkState)){
                        synchronized(linkStates){
        ...
        private void computeRoutingTable() {
                ...
                HashMap<Integer, RoutingEntry> newList = new HashMap<Integer, RoutingEntry>();
                newList.clear();
                HashMap<Integer, RoutingEntry> tempList = new HashMap<Integer, RoutingEntry>();
                newList.put(this.addr, new RoutingEntry(0, this.addr));
                synchronized(linkStates){
                ...
                newList.put(thisNeighbors[0], tempList.get(thisNeighbors[0]));
                ...
                while (true) {
                        ...
                        for (int i = 0; i < neighbors.length; i++) {
                                ...
                                RoutingEntry neighborRouteEntry = newList.get(neighbor);
                                RoutingEntry neighborTempRouteEntry = tempList.get(neighbor);
                                int costToNext = newList.get(next).cost;
                                int nextHop = newList.get(next).nextHop;
                        ...
                        if (tempList.isEmpty()) {
                                ...
                                newList.put(minCostNode, tempList.get(minCostNode));
                                next = minCostNode;
                ...
                if(newList == null)
                {
                  this.logOutput("newList is null");
                }
